...
public static class InteractiveConfigure
{
    ...
    {
        _tempFolder = Path.Combine(Path.GetTempPath(), "Nedordle_DatabaseCreator_" + RandomExtensions.New().NextString(10));
        Directory.CreateDirectory(_tempFolder);
    }

    private static void Cleanup() => DirectoryExtensions.DeleteWithFiles(_tempFolder);
    ...
    private static void ConfigureLocales()
    {
        AnsiConsole.Status().Spinner(Spinner.Known.Dots2).Start("Preparing to import locales..", ctx =>
        {
            ...
            foreach (var file in Directory.GetFiles(localesFolder, "*.json", SearchOption.AllDirectories))
            {
                ...
                DatabaseController.ExecuteNonQuery("insert into locales(id, data) values('{0}', '{1}')", Path.GetFileNameWithoutExtension(file), noIndentation);
            }
        });
    }
    ...
    private static void ConfigureLanguages()
    {
        ...
        var supportedLanguages = JsonConvert.DeserializeObject<Dictionary<string, string>>(File.ReadAllText(supportedLanguagesFile));
        ...
        foreach (var s in selected)
        {
            ...
            {
                ZipFile.ExtractToDirectory(Path.Combine(_dataFolder, "Dictionaries", shortName + ".zip"), languageDir);
                ...
                info = JsonConvert.DeserializeObject<LanguageInfo>(File.ReadAllText(infoPath)) ?? throw new InvalidOperationException("Failed to parse \"info.json\".");

                ctx.Status("Reading words..");
                words = JsonConvert.DeserializeObject<Dictionary<string, int>>(File.ReadAllText(Path.Combine(languageDir, info.DictionaryFile)));
            });
            ...
            AnsiConsole.Progress().Start(ctx =>
            {
                ...
                DatabaseController.ExecuteNonQuery($"insert into languages(short, \"full\", native) values(\"{info.ShortName}\", \"{info.FullName}\", \"{info.NativeName}\")");
