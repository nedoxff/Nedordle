...
public static class MessageCreated
{
    public static async Task OnMessageCreated(DiscordClient sender, MessageCreateEventArgs e)
    {
        ...
        {
            Log.Debug("Detected input for game with ID {GameId} from {UserId} ({Username})", game, e.Author.Id, e.Author.Username);
            if (!GameHandler.Handlers.ContainsKey(game))
                ...
                new Task(() => GameHandler.Handlers[game].OnInput(e.Author, e.Message.Content).GetAwaiter().GetResult()).Start();
        }
    }
}